...
def best_first_graph_search(problem, f):
    ...
    while frontier:
        node = frontier.pop()
        if problem.goal_test(node.state):
            # print("{} {} {}".format(node, nofvisited, len(node.path())))
            return node, nofvisited, len(node.path())
        explored.add(node.state)
        nofvisited += 1
        #print(node)
        for child in node.expand(problem):
...
#-------------------------------------------------------------------------------
# A is a pair (a,b). Where a is the jug A content and b is the jug B content
...
f    row r, and a value of -1 means that the c-th column has not been
    ...
    def actions(self, state):
        movements = []
        originalState = list(state)
        for col in range(self.N):
            for row in range(self.N):
                auxstate = list(originalState)
                if originalState[col] != row:
                    auxstate[col] = row
                    movements.append(tuple(auxstate))
        return movements
    ...
    def value(self, state):
        #print("Node: {} value: {}".format(Node(state), self.h(Node(state))))
        maxNumOfConflicts = ((self.N * (self.N-1))/2)
        return maxNumOfConflicts - (self.h(Node(state))/2)
...
#    problem = JugWaterProblem((0, 0))
#   result = breadth_first_tree_search(problem)
#    print(result.path())

if __name__ == '__main__':
    romania_problem = GraphProblem('Arad', 'Bucharest', romania_map)
    res = uniform_cost_search(romania_problem)
    print(res.path())
    res2 = breadth_first_graph_search(romania_problem)
    print(res2.path())
    res3 = depth_first_tree_search(romania_problem)
    print(res3.path())
